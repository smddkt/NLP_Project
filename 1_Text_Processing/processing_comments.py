...
# CSV 파일 로드
try:
    df = pd.read_csv('Comments_Edit.csv')
...
def preprocess(comments_content):
    # 공백과 개행 문자 정리
    comments_content = re.sub(r'\s+', ' ', comments_content)  # 모든 공백(개행 포함)을 공백 한 칸으로 치환
    comments_content = comments_content.strip()  # 문자열 양 끝의 공백 제거
    
    # URL 제거
    comments_content = re.sub(r'https?://\S+|www\.\S+', '', comments_content)
    
    # HTML 태그 제거
    comments_content = re.sub(r'<.*?>', '', comments_content)
    
    # 이메일 주소 제거
    comments_content = re.sub(r'\S+@\S+', '', comments_content)
    
    # 소문자 변환
    comments_content = comments_content.lower()
    
    # 숫자 및 단위 보존
    comments_content = re.sub(r'(\d+(?:\.\d+)?\s*(?:mg|ml|pills?|capsules?|tablets?|oz|mcg))', r' \1 ', comments_content)
    
    # 토큰화
    words = word_tokenize(comments_content)
...
if 'body' in df.columns:  
    df['processed_comment'] = df['body'].apply(preprocess) 
    df['dosages'] = df['processed_comment'].apply(extract_dosage_and_supplement)
    print("전처리 및 용량 추출이 완료되었습니다.")
    
    # 전처리된 데이터 및 용량 정보를 새로운 CSV 파일로 저장
    df.to_csv('processed_insomnia_comments_with_dosages.csv', index=False)
    print("전처리된 데이터와 용량 정보가 'processed_insomnia_comments_with_dosages.csv' 파일로 저장되었습니다.")
else:
    print("컬럼 'body'가 데이터프레임에 없습니다. CSV 파일의 컬럼 이름을 확인하세요.")